...
class InvestigatorService:
    ...
    async def _run_loop(self, incident: IncidentResponse) -> None:
        ...
        while True:
            ...
            if not calls:
                outcome = self._parse_outcome(str(response.get("output_text", "")))
    ...
    async def _response(self, inputs: list[dict[str, object]]) -> dict[str, Any]:
        ...
        # SDK parameter types cannot represent the dynamic JSON Schema generated by Pydantic.
        response = await self._client.responses.create(  # type: ignore[call-overload]
            ...
            text={
                "format": {
                    ...
                    "strict": True,
                    "schema": InvestigationOutcome.model_json_schema(),
                }
            },
        )
        payload = cast(dict[str, Any], response.model_dump())
        # ``output_text`` is a computed SDK property rather than a serialized model field.
        # Preserve it explicitly before parsing the structured terminal response.
        payload["output_text"] = response.output_text
        return payload
    ...
    async def _finish(self, incident_id: str, outcome: InvestigationOutcome) -> None:
        if outcome.status == "FAILED":
            await self._incident_service.investigation_failed(
                incident_id, "Codex reported an investigation failure."
            )
            return
