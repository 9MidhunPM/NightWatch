...
const reads=[/^world$/, /^health$/, /^host(?:\/(?:metrics|capabilities))?$/, /^docker\/(?:status|containers(?:\/[\w.-]+)?|networks)$/, /^topology$/, /^incidents(?:\/[\w-]+)?$/, /^resources$/, /^agent\/(?:status|conversations(?:\/[\w-]+)?)$/, /^observability\/beszel$/, /^reports\/current(?:\.csv)?$/, /^deployments\/(?:status|repositories(?:\/[\w.-]+\/[\w.-]+\/branches)?|plans|project-plans(?:\/[\w-]+)?)$/];
...
async function proxy(req:NextRequest,{params}:{params:Promise<{path:string[]}>}){
 ...
 const url=backendURL(path);for(const key of ['conversation_id']){const value=req.nextUrl.searchParams.get(key);if(value)url.searchParams.set(key,value);}
